		...
		muteAudio: {
			...
			callback: async (action) => {
				if (action.options.mute) {
					...
					this.sendPostRequest(`live_events/${await this.parseVariablesInString(action.options.id)}/unmute_audio`, unmute)
				}
			},
		},
		setAudioGain: {
			...
			callback: async (action) => {
				...
				this.sendPostRequest(`live_events/${await this.parseVariablesInString(action.options.id)}/adjust_audio_gain`, audioGain)
			},
		},
		insertSCTE35Message: {
			name: 'Insert SCTE35',
			options: [
				actionOptions.id,
				actionOptions.duration,
			],
			callback: async (action) => {
				let spliceMessage = { cue_point: { duration: action.options.duration, splice_offset: 0 } }
				this.sendPostRequest(`live_events/${await this.parseVariablesInString(action.options.id)}/cue_point`, spliceMessage)
			},
		},
	}
	return actions
